...
import type { GlobalConfig, ScannerConfig } from "./typedefs.js";
...
const getCentralConfig = async (scannerConfig: ScannerConfig, octokitCentral?: Octokit) => {
	...
	const scannerType = "dockerscan";
	const useCentralAllowlist = scannerConfig.spec?.centralAllowlist ?? true;

	if (!useCentralAllowlist) return undefined;

	const centralScannerConfig = await getExternalScannerConfig("central-allowlist", scannerType, octokitCentral);
...
const getGlobalConfig = async (scannerType: string, octokitExternal?: Octokit, octokitCentral?: Octokit) => {
	...
	if (localConfig === undefined) return null;
